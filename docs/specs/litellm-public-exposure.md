...
- **GPT-5.5 / GPT-5.4** → `openai.gpt-5.5` / `openai.gpt-5.4`. **Bedrock Mantle only** (Responses API, endpoint `https://bedrock-mantle.{region}.api.aws/openai/v1`, Bearer `BEDROCK_MANTLE_API_KEY` — NOT SigV4, NOT the `bedrock/` path, NOT Converse/Invoke). Added a separate `bedrock.mantle` config section + a `bedrock_mantle/` template branch + `BEDROCK_MANTLE_API_KEY` env var.
