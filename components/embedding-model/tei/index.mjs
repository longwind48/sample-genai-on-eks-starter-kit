...
export const name = "Text Embedding Inference (TEI)";
...
export async function install() {
  ...
  const { models } = config["embedding-model"]["tei"];
  await utils.model.addModels(models, "embedding-model", "tei");
}

export async function uninstall() {
  const { models } = config["embedding-model"]["tei"];
  await utils.model.removeAllModels(models, "embedding-model", "tei");
